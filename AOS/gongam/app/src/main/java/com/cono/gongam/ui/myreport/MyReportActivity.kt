...
import com.cono.gongam.ui.myreport.ui.theme.GongamTheme
...
@Preview(showBackground = true)
@Composable
fun PreviewWeekChart() {
    val fakeStudyData = StudyDates(
        stopwatchStudyTime = 50,
        timerStudyTime = 50,
        totalStudyTime = 100
    )
    val fakeDataList = mutableListOf<Pair<Int, StudyDates>>().apply {
        for (i in 0 until 7) {
            add(Pair(i, fakeStudyData))
        }
    }
    WeekChart(fakeDataList)
}

@Preview(showBackground = true)
@Composable
fun PreviewMyReportActivity() {
    MyReportActivityScreen()
}